class Insured::GroupSelectionController < ApplicationController
  ...
  def create
    ...
    create_group_selection(keep_existing_plan, @hbx_enrollment, @employee_role, @family, @market_kind)

    hbx_enrollment.coverage_kind = @coverage_kind

    # Set effective_on if this is a case of QLE with date options available.
    hbx_enrollment.effective_on = date_format(params[:effective_on_option_selected]) if params[:effective_on_option_selected].present?
  ...
  def initialize_common_vars
    ...
    if params[:employee_role_id].present?
      @employee_role = select_employee_role(@person, params[:employee_role_id])
