class Insured::GroupSelectionController < ApplicationController
  before_action :initialize_common_vars, only: [:create, :new, :terminate_selection]
 
  def new
    set_bookmark_url
 
    @employee_role = @person.active_employee_roles.first if @employee_role.blank? and @person.has_active_employee_role?
    @market_kind = Forms::GroupSelection.select_market(@person, params[:market_kind])
  ...
  def initialize_common_vars
    # Forms::GroupSelection.set_variables(params)
    ...
    if params[:employee_role_id].present?
      ...
      @employee_role = Forms::GroupSelection.get_employee_role(@person, params[:employee_role_id])
      @role = @employee_role
