...
describe MergeEeAndErWithBothHaveUserAccount do

  let(:given_task_name) { "merge ee and er with_both_have_user_account" }
  subject { MergeEeAndErWithBothHaveUserAccount.new(given_task_name, double(:current_scope => nil)) }
  ...
  describe "merge ee and er roles" do
    ...
    let!(:user2) { FactoryGirl.create(:user, person: person)}
    ...
    context "giving a new state" do
      it "should assign user to the employee" do
        ...
        expect(person.user.id).to eq user2.id
